...
with col1:
    ups_power_kva = st.number_input("UPS névleges [kVA]", min_value=10.0, max_value=200.0, value=10.0)
    backup_time_min = st.number_input("Áthidalási idő [perc]", min_value=5, max_value=60, value=10)
    

with col2:
    load_kw = st.number_input("Terhelés [kW]", min_value=2.0, max_value=200.0, value=10.0)
    power_factor = st.number_input("Power Factor", min_value=0.9, max_value=1.0, value=1.0)
    

with col3:
    initial_battery_type = st.selectbox("Akkumlátor típus:", BATTERY_LIST)
    stringcount = st.selectbox("Stringek száma[db]", stringlist)
...
if st.button("Számítás"):
    if load_kw > ups_power_kva * power_factor:
        ...
        st.markdown(f"**✅ Valós áthidalási idő :** :red[{actual_time:.2f}] perc (String:{stringcount} x{battery_count}db Össz:{stringcount*battery_count})")
