...
with col1:
    ups_power_kva = st.number_input("UPS névleges [kVA]", min_value=10.0, max_value=250.0, value=10.0, step=0.1,
...
with col3:
    ...
    if manufacturer == "Sunstone":
        ...
        initial_battery_type = st.selectbox("Akkumlátor típus:", BATTERY_LIST_YUASA, index=0, help="Akkumulátor kapacitások /Ah. (1,6V/cell)")
...
with col2:
    load_kw = st.number_input("Terhelés [kW]", min_value=5.0, max_value=250.0, value=9.0,step=0.1,
...
with col4:
    ...
    if battery_count <= 36:
        st.toast("Figyelem: ennél az akkuszámnál kevesebb nem lehet(SOCOMEC) / csökkenhet a DC hatásfok! <95%.",
