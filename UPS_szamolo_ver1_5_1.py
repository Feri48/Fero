...
BATTERY_DATA_YUASA = {
   "SWL280 (7.8Ah)": (444, 280, 140, 100, 50, 30, 20, 10),  # 140W 15perc től becsült értékek
...
with col1:
    ups_power_kva = st.number_input("UPS névleges [kVA]", min_value=1.0, max_value=250.0, value=10.0, step=0.1,
...
with col3:
    ...
    if manufacturer == "Sunstone":
        initial_battery_type = st.selectbox("Akkumlátor típus:", BATTERY_LIST_SUNSTONE, index=0, help="Akkumulátor kapacitások /Ah. (1,6V/cell)")
        battery_data = BATTERY_DATA_SUNSTONE
        time_options = TIME_OPTIONS_SUNSTONE

    else:
        initial_battery_type = st.selectbox("Akkumlátor típus:", BATTERY_LIST_YUASA, index=1, help="Akkumulátor kapacitások /Ah. (1,6V/cell)")
...
with col2:
    load_kw = st.number_input("Terhelés [kW]", min_value=1.0, max_value=250.0, value=9.0,step=0.1,
                              help="A valós terhelés kW-ban (max:250kW).")
    power_factor = st.number_input("Teljesítménytényező)", min_value=0.89, max_value=1.00, value=1.00, step=0.01,
...
with col4:
    ...
    battery_count = st.slider("Akkumlátorok száma", min_value=32, max_value=44, value=40,
                              help="Akkuk száma egy stringben 32-44 között.")    
    # if  power_factor >= 0.95 and
    if battery_count <= 36:
        st.toast("Figyelem: ennél az akkuszámnál csökkenhet a DC hatásfok! <95%.",
                 icon="⚠️")  # Ha a battery_count <= 36, akkor a power_factor automatikusan 0.95-re állítódik
        # st.error("⚠️ A hatásfok az akkuk  száma miatt 95%!")
