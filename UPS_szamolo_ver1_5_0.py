...
with col1:
    ups_power_kva = st.number_input("UPS névleges [kVA]", min_value=10.0, max_value=200.0, value=10.0,help="Látszólagos teljesítmény kVA-ban.")
    ...
    #calc_button = st.button("Számítás")  # <-- ide került át

with col2:
    load_kw = st.number_input("Terhelés [kW]", min_value=2.0, max_value=200.0, value=8.5, help="A valós terhelés kW-ban.")
    power_factor = st.number_input("Teljesítménytényező [cos φ]", min_value=0.89, max_value=1.0, value=1.0,help="Jellemzően 0.9-1.0 között")
    

with col3:
    initial_battery_type = st.selectbox("Akkumlátor típus:", BATTERY_LIST, help="Akkumulátor kapacitások /Ah.")
    stringcount = st.selectbox("Stringek száma[db]", stringlist,help="A stringek száma.")
    battery_count = st.slider("Akkumlátorok száma", min_value=32, max_value=44, value=40, help="Akkuk száma egy stringben.")

# Az UPS maximális leadható valós teljesítménye (kW)
if load_kw > ups_power_kva * power_factor:
    st.error(f"Az UPS terhelése:({load_kw / power_factor:.2f} kW) nagyobb, mint a maximális teljesítménye:({ups_power_kva:.2f} kVA)! cos φ=({power_factor:.2f})! ")
    #st.error(f"Figyelem a teljesítménytényező cos φ=({power_factor:.2f})!")
else:
    ...
    st.markdown(f"**✅ Valós áthidalási idő : :red[{minutes}] perc :red[{seconds}] mp (String:{stringcount}x{battery_count}db össz:{stringcount*battery_count}db**)")  
    ...
    st.markdown(f"**Leadott teljesítmény veszteséggel(DC) (eff = 0.95):** {load_kw / EFFICIENCY:.2f} kW")
    st.markdown(f"**Akkumlátor teljesítmény 1 db-ra ({backup_time_min} percnél):** {power_per_batt:.2f} W - {power_per_batt/6:.2f} W/cella ")
...
st.markdown(
    ...
    <marquee behavior="scroll" direction="left" scrollamount="6">
        🔋 UPS méretező | Készítette: Ferosoft ™®| ©2025 V1.0.0| Minden jog fenntartva !💡
    </marquee>
    </div>
    """,
    unsafe_allow_html=True
)
#  3 colum verz. müködőképes verzió: 1.5.0 file  Real -time
# 2025.06.05.
# python -m streamlit run UPS_szamolo_ver1_5_0.py
